...
def build_regions(level_name: str, player: int, multiworld: MultiWorld) -> List[JakAndDaxterRegion]:
    ...
    main_area.add_cell_locations([96], access_rule=lambda state: can_trade(state, player, multiworld, 1530))
    main_area.add_cell_locations([97], access_rule=lambda state: can_trade(state, player, multiworld, 1530, 96))
    main_area.add_cell_locations([98], access_rule=lambda state: can_trade(state, player, multiworld, 1530, 97))
    main_area.add_cell_locations([99], access_rule=lambda state: can_trade(state, player, multiworld, 1530, 98))
    main_area.add_cell_locations([100], access_rule=lambda state: can_trade(state, player, multiworld, 1530))
    main_area.add_cell_locations([101], access_rule=lambda state: can_trade(state, player, multiworld, 1530, 100))
